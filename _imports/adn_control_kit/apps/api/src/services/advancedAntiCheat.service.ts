...
export async function applyRiskOutcome(userId: string, risk: number, signals: string[]) {
  ...
  const lockUntil =
    risk >= 50 ? new Date(Date.now() + 60 * 60 * 1000) :
    risk >= 30 ? new Date(Date.now() + 15 * 60 * 1000) :
    null;
  ...
  if (signals.length > 0) {
    await prisma.fraudReviewCase.create({
      data: {
        userId,
        severity: risk >= 50 ? "high" : risk >= 30 ? "medium" : "low",
        title: "Automated fraud signal batch",
        summary: signals.join(", "),
        riskSignalsJson: JSON.stringify({ risk, signals }),
      },
    });
  }
}
